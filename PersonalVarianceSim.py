...
# -*- coding: utf-8 -*-
"""
Personal Variance Simulator — Based on PostContestSim.py
Same simulation engine, but additionally saves raw per-user iteration outcomes
and auto-generates variance analysis data (bankroll trajectories, drawdown stats).

Run this instead of PostContestSim.py when you want variance graph data.
After simulation completes, it automatically runs PublishVariance.py to produce
variance_data.json for the variance.html dashboard.

All original PostContestSim outputs (CSVs, portfolio percentiles) are still produced.
"""
...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Save raw per-user outcomes for variance analysis (bankroll simulations)
        raw_outcomes_path = os.path.join(out_dir, f"{base_stem}_user_outcomes.npz")
        np.savez_compressed(
            raw_outcomes_path,
            outcomes=combined_outcomes,           # (total_iters, num_users)
            per_contest=combined_per_contest,     # (K, total_iters, num_users)
            user_names=np.array(user_display_names, dtype=object),
            contest_names=np.array(contest_names, dtype=object),
        )
        log(f"[done] wrote raw user outcomes: {raw_outcomes_path} ({combined_outcomes.shape[0]:,} iters x {num_users} users)")
        ...
        # Auto-run variance publisher for DHollis24
        log("\n" + "=" * 60)
        log("Running variance analysis for DHollis24...")
        log("=" * 60)
        try:
            from PublishVariance import generate_variance_data
            variance_result = generate_variance_data(
            ...
            if variance_result:
                variance_out = os.path.join(out_dir, "variance_data.json")
                with open(variance_out, 'w', encoding='utf-8') as vf:
                    json.dump(variance_result, vf)
                log(f"[done] Variance data: {variance_out}")
                log(f"  Paths: {variance_result['config']['num_paths']} x {variance_result['config']['path_length']} events")
                log(f"  Single-event EV: ${variance_result['single_event']['mean']:.2f}")
                log(f"  Avg max drawdown: ${variance_result['drawdown_stats']['mean']:,.2f}")
                log(f"  Avg max upswing: ${variance_result['upswing_stats']['mean']:,.2f}")
                log(f"\n  Open variance.html in your browser to view the dashboard!")
            else:
                log("[warn] Could not generate variance data (user not found?)")
        except Exception as ve:
            log(f"[warn] Variance generation failed: {ve}")
            log("  You can run PublishVariance.py manually after simulation completes.")
    try:
        input("Press Enter to close...")
    except Exception:
        pass
